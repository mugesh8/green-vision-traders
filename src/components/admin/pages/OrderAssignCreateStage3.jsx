    ...
    const loadData = async () => {
      try {
        ...
        try {
          ...
          // Parse stage2_data to get labour data
          if (assignmentData.stage2_data) {
            try {
              const stage2Data = typeof assignmentData.stage2_data === 'string'
              ...
              productAssignments.forEach(pa => {
                if (pa.id && pa.labourName) {
                  if (!stage2LabourMap[pa.id]) {
                    stage2LabourMap[pa.id] = pa.labourName;
                  }
                }
              });
            } catch (e) {
          ...
          // Parse stage3_assignments to get saved stage3 data
          if (assignmentData.stage3_assignments) {
            try {
              stage3Products = typeof assignmentData.stage3_assignments === 'string'
                ? JSON.parse(assignmentData.stage3_assignments)
                : assignmentData.stage3_assignments;
            } catch (e) {
              console.error('Error parsing stage3_assignments:', e);
            }
          }
        } catch (error) {
          ...
          if (stage3Products && stage3Products.length > 0) {
            ...
            rows = orderData.items.map((item) => {
              ...
              return {
                ...
                grossWeight: `${item.gross_weight || 0} kg`,
